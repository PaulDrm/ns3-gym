...
if noise: 
    ...
    agent = agents.llm_agent_open_reasoning_noise(model, tokenizer, device, generate_config_dict=generation_args)
else:
    agent = agents.llm_agent_outline(model, tokenizer, device, generate_config_dict=generation_args)

warm_ups = [0]#[0, 10, 20] #0,10,
max_lengths = [10]#[60]#, 20, 30] #10, 20, 40]#[0, 10, 20] #0,10, #,30,40
